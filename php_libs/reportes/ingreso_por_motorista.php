...
function FancyTable($header)
{
    ...
    if($print_no_header === 0){
        ...
        $w=array(15,90,20,20,35); // RUTA, PASAJES, PRECIO UNITARIO, INGRESOS CANTIDAD DE BUSES. DESCRIPCION, VALOR, CONCEPTO//determina el ancho de las columnas
...
//Títulos de las columnas
    $header=array('Código','Nombre','P/U','Cant.T/V','Ingresos');	
//Títulos de las columnas
    $pdf->AliasNbPages();
    $pdf->AddPage();
...
// Diseño de Lineas y Rectangulos.
	...
	// estado de cuenta
	$pdf->RoundedRect(15, 45, 135, 8, 2, '1234', 'DF');
	$pdf->RotatedText(18,50,"REPORTE DE INGRESO POR MOTORISTA DEL 1 AL $numero_dias/$fecha_partial[1]/$fecha_partial[0]",0);
// Definimos el tipo de fuente, estilo y tamaño.
...
//  DATOS NECESARIOS PARA CATALOGO RUTA
    ...
    $query = "SELECT codigo, btrim(nombres || CAST(' ' AS VARCHAR) || apellidos) AS nombre_completo FROM personal ORDER BY codigo";
    // Ejecutamos el Query.
    ...
    while($listado = $consulta -> fetch(PDO::FETCH_BOTH))
    {
        ...
        $codigo_tc = $listado['codigo'];
        $nombre_motorista = trim($listado['nombre_completo']);
        //$numero_placa = trim($listado['numero_placa']);
        /****************************************************************************************** */
            // armando el Query. PARA LA TABLA CATALOGO TIQUETE COLOR.
           $query_tc = "SELECT SUM(pro.total_ingreso) AS total_ingreso_por_bus,
                per.codigo, pro.codigo_tiquete_color, cat_tc.precio_publico
                    FROM produccion pro
                        ...
                        INNER JOIN catalogo_tiquete_color cat_tc ON cat_tc.id_ = pro.codigo_tiquete_color
                            WHERE pro.codigo_personal = '$codigo_tc' AND pro.fecha >= '$fecha_partial[0]-$fecha_partial[1]-01' AND pro.fecha <= '$fecha_partial[0]-$fecha_partial[1]-$numero_dias'
                                GROUP BY per.codigo, pro.codigo_tiquete_color, cat_tc.precio_publico";
            ...
            while($listado_tc = $consulta_tc -> fetch(PDO::FETCH_BOTH))
            {
                ...
                $sub_total_tiquete = $sub_total_tiquete + $cantidad_tiquete_vendido;
                    ...
                    {
                        $pdf->SetX(15);
                        $pdf->Cell($w[0],$h[0],$codigo_tc,0,0,'L',$fill);    // N.º Unidad
                        $pdf->Cell($w[1],$h[0],$nombre_motorista,0,0,'L',$fill);    // N.º Placa
