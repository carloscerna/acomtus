        ...
        while($row = $consulta -> fetch(PDO::FETCH_BOTH))
            {
                ...
                //  CALCULO DEL SALARIO MENSUAL, DIARIO Y POR HORA.
                    ...
                    $pago4Horas = round($pago_diario_hora * 4,10);     // CONVIERTE A PAGO DIARIO POR HORA 1T
                ...
                // CREAR ARRAY ASOCIATIVA. SALARIO.Ç
                    ...
                    $salario["DescuentoCastigo"] = 0;
                    $salario["DescuentoISSS"] = 0;
                    $salario["DescuentoPP"] = 0;
                    $salario["SinPunteo"] = 0;
        ...
        if($consulta_asistencia -> rowCount() != 0){
            while($listado = $consulta_asistencia -> fetch(PDO::FETCH_BOTH))
                {
                                    ...
                                    switch ($JornadaLicenciaPermiso){                            
                                        ...
                                        case 'A':  //  asueto
                                            ...
                                            $JornadaCodigoExtra = $CodigoNombreJornada["DescripcionAsueto"][$fila_array];
                                                JornadaExtra($JornadaCodigoExtra);
                                        break;
                                        case 'TA':  // trabajo  asueto
                                            // Revisar si hay jornada Extra TRABAJO VACACION
                                            $JornadaCodigoExtra = $CodigoNombreJornada["DescripcionAsueto"][$fila_array];
                                                JornadaExtra($JornadaCodigoExtra);
                                        break;
                                        case 'DA':  // trabajo descanso asueto
                                            // Revisar si hay jornada Extra TRABAJO VACACION
                                            $JornadaCodigoExtra = $CodigoNombreJornada["DescripcionAsueto"][$fila_array];
                                                JornadaExtra($JornadaCodigoExtra);
                                        break;
                            ...
                            case "A":
                                // colocar la A de ASueto
                                    //$JornadaAsueto = "A";
                                        ...
                                        // Revisar si hay jornada Extra TRABAJO asuento
                                            $JornadaCodigoExtra = $CodigoNombreJornada["DescripcionAsueto"][$fila_array];
                                                JornadaExtra($JornadaCodigoExtra);
                            break;
                            case "VACIO":
                    ...
                    // BUSCAR LA CANTIDAD DE 1T Ó 1.5T PARA OBTENER EL EXTRA. DE UNA JORNADA DE DESCANSO.
                        foreach ($ValoresCount['CantidadDescripcionAsueto'] as $Jornada => $cantidad) {
                            if($Jornada == "4H"){
                                $salario["ExtraAsueto"] = $salario["ExtraAsueto"] + ($salario["Extra4H"] * $cantidad);
                            }
                            if($Jornada == "1T"){
                                $salario["ExtraAsueto"] = $salario["ExtraAsueto"] + ($salario["Extra1T"] * $cantidad);
                            }
                            if($Jornada == "1.5T"){
                                $salario["ExtraAsueto"] = $salario["ExtraAsueto"] + ($salario["Extra15T"] * $cantidad);
                            }
                        }
            ...
            //  CONDICIONAR PARA RELLENAR CON DATOS O SIN DATOS DE CALCULO.
                if($CalcularDatos == "si"){
                    ...
                    # PRESENTAR ASUETO.1
                        $asueto_pantalla =  number_format($salario["ExtraAsueto"],2,'.',',');
                    ...
                    if($DepartamentoEmpresa == $NombresCodigoDE["Motorista"] || $DepartamentoEmpresa  == $NombresCodigoDE["Revisador"]){
                        ...
                        $salario_extra_pantalla = number_format($salario["Extra"] + $salario["ExtraAsueto"],2,'.',',');
                        $pdf->Cell($w[1],6,'$' . $salario_extra_pantalla,1,0,'C',$fillaFila);
                    }
                    ...
                    # POR DEPARTAMENTO
                        if($DepartamentoEmpresa == $NombresCodigoDE["Motorista"] || $DepartamentoEmpresa  == $NombresCodigoDE["Revisador"]){
                            $salario["TotalExtra"] = $salario["HoraExtraDiurna"] + $salario["ExtraAsueto"];
                            ...
                            $salario["TotalExtra"] = $salario["Extra"] + $salario["ExtraAsueto"];
                            $salario_total_extra_pantalla = number_format($salario["TotalExtra"],2,'.',',');
                            $pdf->Cell($w[1],6,'$' . $salario_total_extra_pantalla,1,0,'C',$fillaFila);
                        }
    ...
    global $pdf, $fill, $w, $salario, $FechaDDT;
        ...
        if($JornadaLicenciaPermiso == "PP" || $JornadaLicenciaPermiso == "ISSS" || $JornadaLicenciaPermiso == "SP"){
            ...
            if($JornadaLicenciaPermiso == "SP"){
                ...
                $fill = true;
                // Acumular el valor de un día. "Descuento"abort
        ...
        // IMPRIMIRVALORES
        if($JornadaLicenciaPermiso == "A"){$JornadaLicenciaPermiso = $JornadaLicenciaPermiso . ".";}
...
function VerificarFechaDescuento($codigo_personal){
   ...
   //print $codigo_personal . "<br>"; var_dump($FechaDDT);
    ...
    // si es motorista catalogo_departamento-empresa 02
        $primerDias = []; $ultimoDias = []; $BuscarFechaInicio = []; $BuscarFechaFin = []; $ll = 0;
    ...
    // PARA DETERMINAR SI EXISTE:
        /*  F - FALTA.
            ...
            4H - MAS...
        */
    for ($bloqueSemana=0; $bloqueSemana < count($BuscarFechaInicio) ; $bloqueSemana++) { 
        ...
        // armanr query para buscar si existe la fecha en el perido seleccionar en la tabla personal asisitencia.
            $queryAsistenciaPorSemana= "SELECT pa.fecha, pa.codigo_jornada, pa.codigo_tipo_licencia, pa.codigo_jornada_asueto, pa.codigo_jornada_vacaciones,
                    ...
                    cat_lp.descripcion as descripcion_licencia, cat_lp.horas as horas_licencia
                        ...
                        INNER JOIN catalogo_jornada cat_j ON cat_j.id_ = pa.codigo_jornada
                        INNER JOIN catalogo_tipo_licencia_o_permiso cat_lp ON cat_lp.id_ = pa.codigo_tipo_licencia
                            WHERE pa.codigo_personal = '$codigo_personal' and pa.fecha >= '$BuscarFechaInicio[$bloqueSemana]' and pa.fecha <= '$BuscarFechaFin[$bloqueSemana]' order by pa.fecha";
            $consultaAsistenciaPorSemana = $dblink -> query($queryAsistenciaPorSemana);
            // validar si existen archivos en la consulta segun la fecha.
            $cantidadDiasPorSemana = $consultaAsistenciaPorSemana -> rowCount();
            // regrear la variable conteo a 0
                $CantidadC = []; $CantidadF = []; $Cantidad4H = []; $CantidadPP = []; $CantidadISSS = []; $Fecha = [];
                //$conteo_4h = 0; $descuento = 0; $CantidadF = 0; $CantidadPP = 0; $CantidadH = 0; $Descuento4H = 0;
            // Verificar si existen registros.
                if($consultaAsistenciaPorSemana -> rowCount() != 0){
                    while($DiasJornadaLicencias = $consultaAsistenciaPorSemana -> fetch(PDO::FETCH_BOTH))
                    {
                        $Jornada = trim($DiasJornadaLicencias['descripcion_jornada']);
                        $Licencia = trim($DiasJornadaLicencias['descripcion_licencia']);
                        $Fechas = trim($DiasJornadaLicencias['fecha']);
                        //
                        if($Fechas >= $fecha_periodo_inicio && $Fechas <= $fecha_periodo_fin){
                            $CodigoNombreJornadaDDT['DescripcionJornada'][] = $Jornada;
                            $CodigoNombreJornadaDDT['DescripcionLicencia'][] = $Licencia;
                            $Fecha[] = $Fechas;
                       }
                    }   // LAZO WHILE DE LA CONSULTA POR SEMANA.
                    // Fecha de Assitencia diaria obtener el Primer Elemento y el ültimo.
                        $FechaAsistenciaInicio = reset($Fecha);
                    ...
                    if($codigo_personal == '100225'){
                        ...
                        var_dump($Cantidad4H);
                        print "Valores de las matrices de descuento Faltas: <br>";
                        var_dump($CantidadF);
                        print "Valores de las matrices de descuento cASTIGO: <br>";
                        var_dump($CantidadC);
                        print "Valores de las matrices de descuento ISSS: <br>";
                        var_dump($CantidadISSS);
                        print "Valores de las matrices de descuento PP permiso: <br>";
                        var_dump($CantidadPP);
                        print "Valores de las Fechas de Asistencia Diaria: <br>";
                        var_dump($Fecha);
                        print "Valores de las Fechas de Asistencia Diaria Inicio: <br>";
                        var_dump($FechaAsistenciaInicio);
                        print "Valores de las Fechas de Asistencia Diaria Fin: <br>";
                        var_dump($FechaAsistenciaFin);
                    }
                    unset($CodigoNombreJornadaDDT); $CodigoNombreJornadaDDT = [];
