...
if ($print_sumas === 1){
						// Armar query.
						$query_saldo = "select round(sum(fianza),2) as suma_fianza, round(sum(devolucion),2) as suma_devolucion,  round((sum(fianza) - sum(devolucion)),2) as saldo_fianza from fianzas where codigo = '$codigo'";	
								...
								while($sumas = $consulta_saldo-> fetch(PDO::FETCH_BOTH))
									{
										...
										$suma_p= trim($sumas['suma_fianza']);
										$suma_d= trim($sumas['suma_devolucion']);
										$saldo_p = trim($sumas['saldo_fianza']);
...
//Títulos de las columnas
    $header=array('Fecha','Fianzas $','Devoluciones $','Descripción');	
//Títulos de las columnas
    $pdf->AliasNbPages();
    $pdf->AddPage();
...
// Diseño de Lineas y Rectangulos.
	...
	$pdf->RotatedText(18,50,'ESTADO DE CUENTA - FIANZAS',0);
	// CODIGO
...
//  mostrar los valores de la consulta
    $w=array(25,20,30,120); // FECHA, FIANZAS, DEVOLUCION Y DESCRIPCION.//determina el ancho de las columnas
...
// armando el Query. PARA LA TABLA HISTORIAL.
	$query_historial = "SELECT p.id_fianza, p.codigo, p.fecha, p.fianza, p.devolucion, p.descripcion, per.apellidos, per.nombres
			FROM fianzas p
			...
			// cambiar el valor de las variables
				...
				while($listadoHistorial = $consulta_historial -> fetch(PDO::FETCH_BOTH))
					...
					$num++; $salto++;
						...
						$fianza = trim($listadoHistorial['fianza']);
						$devolucion = trim($listadoHistorial['devolucion']);
					...
					if($num === 1){
						...
						$pdf->Cell($w[0],$w2[0],$fianza,'R',0,'R',$fill);
						$pdf->Cell($w[0],$w2[0],$devolucion,'R',0,'R',$fill);
