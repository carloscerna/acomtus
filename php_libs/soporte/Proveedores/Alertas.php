	...
	if(isset($_POST) && !empty($_POST)){
		...
		switch ($_POST['accion']) {
			...
			case 'BuscarAlertas':
				...
				$codigoProveedores = $_POST["codigoProveedores"];
				// Armamos el query.
					    $query = "SELECT cA.id_, cA.codigo_proveedores, cA.codigo_alerta, cat_c_proveedores.descripcion as alerta_descripcion, 
                            cat_c_proveedores.codigo_clasificacion_riesgo, cat_c_riesgos.descripcion as riesgo_descripcion, 
                            cat_c_riesgos.color 
                            FROM proveedores_alertas cA 
                            INNER JOIN catalogo_conductas_clientes_proveedores cat_c_proveedores ON cA.codigo_alerta = cat_c_proveedores.codigo 
                            INNER JOIN catalogo_clasificacion_riesgos cat_c_riesgos ON cat_c_riesgos.codigo = cat_c_proveedores.codigo_clasificacion_riesgo 
									WHERE cA.codigo_proveedores = '$codigoProveedores'";
				// Ejecutamos el Query.
				...
				if($consulta -> rowCount() != 0){
					...
					while($listado = $consulta -> fetch(PDO::FETCH_BOTH))
					{
						...
						$codigo_empleado = trim($listado['codigo_proveedores']);
			...
			case 'guardarAlertas':
				// consultar el registro antes de agregarlo.
					...
					$codigoEmpleado = $_POST['Proveedores'];
					$codigoAlerta = $_POST['lstProveedoresAlertas'];
						$query = "SELECT * FROM proveedores_alertas WHERE codigo_proveedores = '$codigoEmpleado' and codigo_alerta = '$codigoAlerta' ";
				// Ejecutamos el Query.
				$consulta = $dblink -> query($query);
				//
				if($consulta -> rowCount() != 0){
				...
				// proceso para grabar el registro
					$query = "INSERT INTO proveedores_alertas (codigo_proveedores, codigo_alerta) 
			...
			case 'eliminarAlertas':
				// Armamos el query
				$id_ = $_REQUEST["id_"];
					$query = "DELETE FROM proveedores_alertas WHERE id_ = '$id_'";
				// Ejecutamos el query
